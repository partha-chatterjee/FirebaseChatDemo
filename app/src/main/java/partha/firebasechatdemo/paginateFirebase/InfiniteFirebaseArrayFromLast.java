...
public class InfiniteFirebaseArrayFromLast implements ChildEventListener {
    ...
    public InfiniteFirebaseArrayFromLast(Query ref, int numberPerPage) {
...
//        mQuery = ref.limitToFirst(mNumberPerPage + 1).startAt(mNextChildKey);
        mQuery = ref.limitToLast(mNumberPerPage + 1).endAt(mNextChildKey);
...
//        ref.orderByKey().limitToFirst(mNumberPerPage + 1).startAt(mNextChildKey).addChildEventListener(this);
        ref.orderByKey().limitToLast(mNumberPerPage + 1).endAt(mNextChildKey).addChildEventListener(this);
        mCount = 0;
    }
